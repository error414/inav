...
#endif
        // Without the keep-alive there is no replay for bidir: the ports switch direction per
        // frame and one may sit in input capture, so the plain circular replay below does not
        // apply. The ESC sees a frame gap for the flash write, as in Betaflight.
        return;
    }
#endif
...
#ifdef USE_DSHOT_BIDIR
// Called at the end of init, once every other DMA user has claimed its streams, so the
// keep-alive streams are allocated (and a failure logged) at boot rather than at the first
// save. dshotBidirKeepaliveStart() still calls the same init as a fallback.
void pwmDshotKeepaliveInit(void)
{
#ifdef USE_DSHOT_BIDIR_KEEPALIVE
    if (useDshotTelemetry) {
        dshotBidirKeepaliveInit();
    }
#endif
}
#endif
